...
4. FIRST STEP: MUST use open_app(app_name) to open target app. NEVER swipe on first step to find app.
5. Opening apps: ALWAYS try open_app first. open_app(deeplink) > open_app(name) >> manually find on desktop.
6. Finding app on desktop (ONLY if open_app FAILED and reported in history): Swipe LEFT to find search bar, NOT swipe up to open app drawer
7. MAX 3 consecutive swipes in same direction. If already at boundary (no change after swipe), STOP and try different approach.
